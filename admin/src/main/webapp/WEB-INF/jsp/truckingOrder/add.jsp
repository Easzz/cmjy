...
<div class="wrapper wrapper-content animated fadeInRight">
	...
	<form class="form-horizontal m-t" id="truckingOrderForm" action="${projectPath }/truckingOrder/add.action"
		  method="post">

		<div class="form-group">
			<label class="col-sm-3 control-label">发车时间</label>
			<div class="col-sm-3">
				<input type="text" name="departTime" id="departTime" class="form-control" required="required">
			</div>
		</div>
			...
			<div class="col-sm-3">
				<select name="transportRouteId" class="form-control" required="required">
					<c:forEach items="${transportRouteCustoms}" var="transportRouteCustom">
						<option value="${transportRouteCustom.id}">${transportRouteCustom.displayName}</option>
					</c:forEach>
				</select>
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-3">
				<input id="passWay"  required="required" name="passWay" class="form-control" type="text">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-3 control-label">牌证情况：</label>
			<div class="col-sm-3">
				<label class="radio-inline i-checks"><input type="radio" name="certificateStatus" value="1"
															required="required">正常</label>
				<label class="radio-inline i-checks"><input type="radio" name="certificateStatus" value="0">异常</label>
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<div class="col-sm-3">
				<label class="radio-inline i-checks"><input type="radio" name="weatherStatus" value="晴"
															required="required">晴</label>
				<label class="radio-inline i-checks"><input type="radio" name="weatherStatus" value="阴">阴</label>
				<label class="radio-inline i-checks"><input type="radio" name="weatherStatus" value="雨">雨</label>
				<label class="radio-inline i-checks"><input type="radio" name="weatherStatus" value="雾">雾</label>
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-3 control-label">道路情况：</label>
			<div class="col-sm-3">
				<label class="radio-inline i-checks"><input type="radio" name="wayStatus" value="干燥" required="required">干燥</label>
				<label class="radio-inline i-checks"><input type="radio" name="wayStatus" value="湿润">湿润</label>
			</div>
		</div>
		<%--  <div class="form-group">
			...
			<div class="col-sm-3">
				<select name="promptId" class="form-control" required="required">
					<c:forEach items="${truckingOrderPromptCustoms}" var="truckingOrderPromptCustom">
						<option value="${truckingOrderPromptCustom.id}">${truckingOrderPromptCustom.title}</option>
					</c:forEach>
				</select>
			</div>
		</div> --%>
